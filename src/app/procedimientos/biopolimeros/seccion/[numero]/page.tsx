...
export default function SeccionPage() {
  ...
  useEffect(() => {
    if (isNaN(numero) || numero < 1 || numero > TOTAL_SECTIONS) {
      router.push("/procedimientos/biopolimeros/seccion/1")
    }
  }, [numero, router])
  ...
  const getPreviousRoute = () => {
    if (numero === 1) {
      return "/procedimientos/biopolimeros/video-intro"
    }
    return `/procedimientos/biopolimeros/seccion/${numero - 1}`
  }

  const getNextRoute = () => {
    if (numero === TOTAL_SECTIONS) {
      return "/procedimientos/biopolimeros/video-final"
    }
    return `/procedimientos/biopolimeros/seccion/${numero + 1}`
  }
